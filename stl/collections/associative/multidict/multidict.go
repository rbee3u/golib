...
func (d *Dict[K, V]) Count(key K) int {
	return d.base.CountMulti(Pair[K, V]{Key: key})
}
...
func (d *Dict[K, V]) EqualRange(key K) (Iterator[Pair[K, V]], Iterator[Pair[K, V]]) {
	lb, ub := d.base.EqualRangeMulti(Pair[K, V]{Key: key})
...
func (d *Dict[K, V]) Insert(key K, value V) Iterator[Pair[K, V]] {
	it := d.base.InsertMulti(Pair[K, V]{Key: key, Value: value})

	return Iterator[Pair[K, V]]{base: it}
}
