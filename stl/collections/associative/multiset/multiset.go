...
func (s *Set[T]) EqualRange(data T) (Iterator[T], Iterator[T]) {
	lb, ub := s.base.EqualRangeMulti(data)
...
func (s *Set[T]) Insert(data T) Iterator[T] {
	it := s.base.InsertMulti(data)

	return Iterator[T]{base: it}
}
